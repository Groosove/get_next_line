...
/*   Updated: 2020/08/12 17:45:02 by flavon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((*first_str = malloc(sizeof(char) * (content->length + 1))) == 0)
		return (-1);
	...
	free(content->line);
	if ((content->line = malloc(sizeof(char) * BUFFER_SIZE)) != 0)
		return (-1);
	return (1);
}
...
static int		ft_recheck_line(t_line *line, char **dst, size_t index)
{
	...
	count = 0;
	if ((*dst = malloc(sizeof(char) * (index + 1))) == 0)
		return (-1);
	while (index > count)
	{
	...
	if ((tmp = malloc(sizeof(char) * (line->length - count - 1))) == 0)
		return (-1);
	length = line->length;
	line->length = 0;
	while (++index < length)
...
static int		ft_list_elem(t_line *line, char **dst)
{
	...
	src = malloc(sizeof(char) * BUFFER_SIZE);
...
int				get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || line == 0)
		return (-1);
	if ((list_elem = ft_list_get_elem(fd, list)) == 0 &&
		(list_elem = ft_list_new_add(fd, &list)) == 0)
	{
		ft_list_clear(list);
		return (-1);
	}
